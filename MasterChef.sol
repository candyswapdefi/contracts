...
import "./libs/ICandyReferral.sol";
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    // Initial emission rate: 1 CANDY per block.
    uint256 public constant INITIAL_EMISSION_RATE = 1000 finney;
    // Minimum emission rate: 0.1 CANDY per block.
    ...
    ICandyReferral public candyReferral;
    ...
    constructor(
        ...
        candyPerBlock = INITIAL_EMISSION_RATE;
    }
    ...
    function deposit(uint256 _pid, uint256 _amount, address _referrer) public nonReentrant {
        ...
        updatePool(_pid);
        if (_amount > 0 && address(candyReferral) != address(0) && _referrer != address(0) && _referrer != msg.sender) {
            candyReferral.recordReferral(msg.sender, _referrer);
        }
        if (user.amount > 0) {
        ...
        if (_amount > 0) {
            ...
            if (address(pool.lpToken) == address(candy)) {
    ...
    function updateEmissionRate() public {
        ...
        require(candyPerBlock > MINIMUM_EMISSION_RATE, "updateEmissionRate: Emission rate has reached the minimum threshold");
        ...
        uint256 newEmissionRate = candyPerBlock;
        ...
        if (newEmissionRate >= candyPerBlock) {
            return;
        }

        massUpdatePools();
        lastReductionPeriodIndex = currentIndex;
        uint256 previousEmissionRate = candyPerBlock;
        candyPerBlock = newEmissionRate;
        emit EmissionRateUpdated(msg.sender, previousEmissionRate, newEmissionRate);
    }

    // Update the candy referral contract address by the owner
    function setCandyReferral(ICandyReferral _candyReferral) public onlyOwner {
        candyReferral = _candyReferral;
    }
    ...
    function payReferralCommission(address _user, uint256 _pending) internal {
        if (address(candyReferral) != address(0) && referralCommissionRate > 0) {
            address referrer = candyReferral.getReferrer(_user);
